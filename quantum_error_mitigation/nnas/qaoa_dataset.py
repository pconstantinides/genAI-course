...
def generate_qaoa_dataset(
    ...
    Parameters:
        ...
        - n_qubits: number of qubits in the QAOA circuit
        - noise_t1_us: T1 time in microseconds (T2 is scaled proportionally
                     to keep the reference ratio, Supp. Sec III.A)
        - partial_training_rate: fraction of training sequences that are
                               truncated to the "hard regime" (Table IV)
        - is_train: whether to generate a training set (variable-length sequences)
        - fixed_L: maximum Trotter step (sequence length) to generate
        - J_over_h: ratio of Ising coupling to transverse field (J/h)
    ...
    sequences = []
    for _ in tqdm(range(n_sequences), desc="Generating QAOA dataset"):
