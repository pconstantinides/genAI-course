...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        Input:
            ...
            t (`int` or `torch.Tensor`): current timestep in a reverse process.
        ...
        mean_coef = 1.0 / alpha_t.sqrt()
        noise_coef = beta_t / (1.0 - alpha_cumprod_t).sqrt()
        mean = mean_coef * (x_t - noise_coef * eps_theta)

        if t.numel() == 1 and t.item() == 0:
            return mean

        sample_prev = mean + sigma_t * torch.randn_like(x_t)
        return sample_prev
