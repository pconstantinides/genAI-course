...
def save_checkpoint(save_dir, ddpm, optimizer, scheduler, step):
    checkpoint = {
...
def load_checkpoint(checkpoint_path, ddpm, optimizer, scheduler, device):
...
def main(args):
    ...
    config.device = 'cpu' if args.gpu == 'cpu' else f"cuda:{int(args.gpu)}"
    ...
    if args.resume_from is not None:
        ...
        if resume_path.exists():
            step = load_checkpoint(resume_path, ddpm, optimizer, scheduler, config.device)
            print(f"Resumed training from {resume_path} at step {step}")
        else:
    ...
    with tqdm(initial=step, total=config.train_num_steps) as pbar:
        while step < config.train_num_steps:
            if step % config.log_interval == 0:
                ...
                save_checkpoint(save_dir, ddpm, optimizer, scheduler, step)
                ddpm.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume_from", type=str, default=None, help="Path to a checkpoint.pt file to resume training")
